...
func sendTokafka(message string,ipport string,topic string) {
        ...
        ipport = strip(ipport,"\"")
        topic = strip(topic,"\"")
        client, err := sarama.NewSyncProducer([]string{ipport}, config)
        if err != nil {
...
func (a *Agent) ZeroSecond(ctx context.Context, waitDuration time.Duration, kafka_ipport string, kafka_topic string) error {
        var wg sync.WaitGroup
        ...
        go func() {
                ...
                for metric := range metricC {
                        ...
                        if err == nil {
//                              fmt.Print("> ", string(octets))
                                message := string(octets)
                                sendTokafka(message,kafka_ipport,kafka_topic)
                        }
                        metric.Reject()
                }
        }()
...
func (a *Agent) newFlush(
        ctx context.Context,
        output *models.RunningOutput,
){
        logError := func(err error) {
                if err != nil {
                        log.Printf("E! [agent] Error writing to output [%s]: %v", output.LogName(), err)
                }
        }
        ...
        for {
                ...
                select {
                case <-ctx.Done():
                        logError(a.newFlushOnce(output, output.Write))
                        return
                default:
                }
                ...
                case <-output.BatchReady:
                        ...
                        logError(a.newFlushOnce(output, output.WriteBatch))
                case <-ctx.Done():
                        logError(a.newFlushOnce(output, output.Write))
                        return
                default:
