...
func (k *Kafka) Write(metrics []telegraf.Metric) error {
	...
	for _, metric := range metrics {
		// 针对日志类型， 完全拼接成一行
		if metric.Name() == "multiline_logparser" {
			msg := ""
			...
			for kt, v := range tmpFields {
				fd := make(map[string]interface{})
				fd[kt] = v
				metric.AddField(kt, v)
				// 增加: 修正metric, 切割多个metric为单个metric
				buf, err := k.serializer.Serialize(metric)
				if err != nil {
					log.Printf("D! [outputs.kafka] Could not serialize metric: %v", err)
					continue
