...
//			go l.to_Stop()
			...
			grapeTimer.NewTickerLoop(1000 * l.Count_interval, -1, l.send_LineCount)
...
func (l *LogParserPlugin)send_LineCount() {
	count := make(map[string]interface{})
	tags := make(map[string]string)
	count["total_count"] = uint64Format(l.Line_count)
	tags["total_files"] = strings.Join(l.Files,",")
	l.acc.AddFields("logparser_win",count,tags,time.Now())
	l.Line_count = 0
}
...
func (l *LogParserPlugin) receiver(tailer *tail.Tail) {
	...
	for line = range tailer.Lines {
		l.Line_count = atomic.AddUint64(&l.Line_count,1)
...
func init() {
	inputs.Add("logparser_win", func() telegraf.Input {
		return NewLogParser()
	})
}
